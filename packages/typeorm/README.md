...
A generic service class (`NestService<T, E>`) that provides:
...
// serviceOptions defaults: { multi: false, softDelete: true, pagination: true, events: true, broadcast: false }
```
...
export class CatsService extends NestService<Cat, CatsEvents> {
  constructor(@InjectRepository(Cat) repo: Repository<Cat>) {
    super(repo, { events: true });   // default; { events: false } opts out
  }
}
```
...
export class CatsEvents extends NestServiceEvents<CatsService, Cat> {
  ...
  async onCreate(cat: Cat, ctx: EventContext<CatsService>) {
